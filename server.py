...
def analyze_shot(data: AnalyzeRequest):
    ...
    prompt = (
        ...
        f"Дай ответ максимум в 3 коротких предложениях (укладывайся в 60 слов суммарно): оцени силу удара "
        f"относительно общего уровня игроков, прокомментируй траекторию и точку приземления, и дай один "
        f"конкретный совет по технике для следующего удара. Обязательно закончи мысль и поставь точку в конце — "
        f"не обрывай ответ на середине предложения. Пиши тепло и подбадривающе, но честно. "
        f"Не используй markdown-разметку, только обычный текст."
    )
